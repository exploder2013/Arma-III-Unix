...
{
	HEADER pHeader;
	pHeader.pID			= this->procID;
	pHeader.imageBase	= 0x00000000;

	DWORD dwBytesRead;
	if (!DeviceIoControl(hDevice, IOCTL_HANDLE, &pHeader, sizeof(HEADER), &pHeader, sizeof(HEADER), &dwBytesRead, NULL))
	{
		return 0;
	}

	return pHeader.imageBase;
}
